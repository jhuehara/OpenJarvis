...
# ── OpenJarvis: run with the AI already installed on this computer ───
# Uses, in order of preference:
#   1. Claude Code CLI (`claude`)  — reuses your Claude login/subscription
#   2. OpenAI Codex CLI (`codex`)  — reuses your ChatGPT login/subscription
#   3. A local model server: Ollama, LM Studio or llama.cpp
# then writes a config for it and launches Jarvis. No API key needed.
#
# Usage:
#   ./scripts/launch-local.sh                      # chat with auto-detected backend
#   ./scripts/launch-local.sh claudecode:opus      # Claude Code with a given model
#   ./scripts/launch-local.sh codex                # force Codex
#   ./scripts/launch-local.sh qwen3.5:9b           # an Ollama model
#   ./scripts/launch-local.sh -- serve             # run `jarvis serve` instead
#   ./scripts/launch-local.sh llama3.1:8b -- ask "Olá"
#
# Environment overrides:
#   LOCAL_LLM_ENGINE   force engine: claude_code | codex | ollama | lmstudio | llamacpp
...
if [[ -z "$ENGINE" ]]; then
    case "$MODEL" in
        claudecode|claudecode:*) ENGINE=claude_code ;;
...
case "$ENGINE" in
    claude_code)
        command -v claude >/dev/null 2>&1 \
            || die "'claude' CLI not found (https://docs.claude.com/en/docs/claude-code/setup)" 4
        [[ -n "$MODEL" ]] || MODEL=claudecode
        ;;
    codex)
        command -v codex >/dev/null 2>&1 \
            || die "'codex' CLI not found (npm install -g @openai/codex)" 4
        [[ -n "$MODEL" ]] || MODEL=codex
        ;;
    ollama)   HOST="$OLLAMA_URL" ;;
    lmstudio) HOST="$LMSTUDIO_URL" ;;
    llamacpp) HOST="$LLAMACPP_URL" ;;
    *) die "unsupported LOCAL_LLM_ENGINE '$ENGINE' (use claude_code, codex, ollama, lmstudio or llamacpp)" 2 ;;
esac
info "engine: $ENGINE${HOST:+ ($HOST)}"
...
elif [[ -n "$HOST" && -z "$MODEL" ]]; then
    MODEL="$(first_openai_model "$HOST")"
    [[ -n "$MODEL" ]] || die "no model loaded in $ENGINE at $HOST — load one first" 5
fi
info "model:  $MODEL"

# ── 3. Write config (refreshed each run; a hand-written file is kept) ─
MARKER="# Generated by scripts/launch-local.sh"
if [[ ! -f "$CONFIG_PATH" ]] || [[ "$(head -n1 "$CONFIG_PATH")" == "$MARKER" ]]; then
...
agent = "simple"
EOF
    info "config: $CONFIG_PATH (written)"
else
